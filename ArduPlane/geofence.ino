...
static void geofence_load(void)
{
    ...
    if (geofence_state == NULL) {
        if (hal.util->available_memory() < 512 + sizeof(struct GeofenceState)) {
            ...
            gcs_send_text_P(SEVERITY_HIGH,PSTR("geo-fence memory overflow")); //added by J2 16/03/14
            goto failed;
        }
        geofence_state = (struct GeofenceState *)calloc(1, sizeof(struct GeofenceState));
        if (geofence_state == NULL) {
            // not much we can do here except disable it
            gcs_send_text_P(SEVERITY_HIGH,PSTR("geo-fence state incorrect")); //added by J2 16/03/14
            goto failed;
        }
    ...
    if (!Polygon_complete(&geofence_state->boundary[1], geofence_state->num_points-1)) {
        ...
        gcs_send_text_P(SEVERITY_HIGH,PSTR("geo-fence first and last points not same")); //added by J2 16/03/14
        goto failed;
    }
    if (Polygon_outside(geofence_state->boundary[0], &geofence_state->boundary[1], geofence_state->num_points-1)) {
        ...
        gcs_send_text_P(SEVERITY_HIGH,PSTR("geo-fence return point outside of polygon fence")); //added by J2 16/03/14
        goto failed;
    }
...
failed:
    ...
    //gcs_send_text_P(SEVERITY_HIGH,PSTR("geo-fence setup error")); //removed by J2 16/03/14
}
